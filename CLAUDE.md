...
**M1–M3 已合并**（PR #2 骨架；PR #9 钱包，checklist F 勾选；PR #19 视频管线，checklist C+D 热度勾选）。关键接入约定：①资金操作只走 `wallet.Service` 的 `XxxTx` 原语同事务注入，撞幂等约束以 `errors.Is(ErrAlreadyGranted)` 识别（事务须整体回滚）；②M5 观看奖励/任务进度的事件 ref = `plays.id`（有效播放上报已预留）；③M4 有效用户判定钩子挂视频审核通过处（`video_count+1` 已同事务）。**下一步：M4——邀请裂变 + 有效用户判定**。实现 M6 前须回旧仓库核对充值超时时长（唯一遗留核对项）。
