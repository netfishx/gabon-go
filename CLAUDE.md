...
**M1 已合并**（PR #2：骨架 + 28 表全量迁移 + 双主体 JWT + 注册登录，E2E 走 httptest + testcontainers 单主缝，`internal/testdb` 为共享测试基建）。**下一步：M2——钱包 + 流水（纯账本，ADR-0006）**；里程碑序列 M1–M7 与工程流程见 `docs/skeleton.md`。实现 M6 前须回旧仓库核对充值超时时长（唯一遗留核对项）。
