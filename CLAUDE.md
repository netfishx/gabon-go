...
**M1、M2 已合并**（PR #2 骨架+迁移+认证；PR #9 钱包五原语+纯账本，checklist F 已勾选）。钱包域是唯一资金入口：消费方发奖/扣款用 `wallet.Service` 的 `XxxTx` 原语同事务注入，撞幂等约束以 `errors.Is(ErrAlreadyGranted)` 识别（事务须整体回滚）。**下一步：M3——视频管线（上传→转码→审核→Feed，重基建：S3/MinIO + ffmpeg + DB 队列）**。实现 M6 前须回旧仓库核对充值超时时长（唯一遗留核对项）。
