...
Issue 在 GitHub Issues（netfishx/gabon-go）追踪，用 gh CLI 读写；外部 PR 也是 triage 入口。见 `docs/agents/issue-tracker.md`。

### Triage labels

五个 triage 角色使用默认命名（needs-triage / needs-info / ready-for-agent / ready-for-human / wontfix）。见 `docs/agents/triage-labels.md`。
