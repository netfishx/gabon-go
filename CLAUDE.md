...
**M1–M4 已合并**（PR #2 骨架；PR #9 钱包，checklist F 勾选；PR #19 视频管线，checklist C+D 热度勾选；PR #27 邀请裂变 + 有效用户判定，checklist B 勾选）。关键接入约定：①资金操作只走 `wallet.Service` 的 `XxxTx` 原语同事务注入，撞幂等约束以 `errors.Is(ErrAlreadyGranted)` 识别（事务须整体回滚）；②M5 观看奖励/任务进度的事件 ref = `plays.id`（有效播放上报已预留）；③有效用户判定原语 `customer.MarkValidIfQualifiedTx`（翻转 CAS + 邀请人行锁 cap 检查 + 同事务发奖），新增触发点须同事务调用；④`video_count` 语义 = 已发布且未删除的作品数（审核 +1 / 删除 −1 对称维护）。**下一步：M5——奖励族（任务 + 签到 + VIP + 广告）**。实现 M6 前须回旧仓库核对充值超时时长（唯一遗留核对项）。
